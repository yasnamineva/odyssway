...
|---|------|-------|--------------|
| 1 | Max 90 days of presence within **any** rolling 180-day window `[D − 179, D]`, both bounds inclusive; no fixed reset | `engine.ts` (`WINDOW_DAYS`, `MAX_DAYS_IN_WINDOW`, `countInWindow`) | Regulation (EU) 2016/399 (Schengen Borders Code), short-stay definition — as restated in the [EC short-stay calculator user manual](https://ec.europa.eu/assets/home/visa-calculator/docs/short_stay_schengen_calculator_user_manual_en.pdf), §1. Exact article/paragraph citation: pending EUR-Lex access (`data/UNVERIFIED/TODO.md`). |
| 2 | Entry day and exit day both count as days of stay | `presence.ts` (inclusive expansion) | Same manual, §1: "The date of entry shall be considered as the first day of stay … the date of exit … the last day of stay." |
| 3 | Days under a residence permit or long-stay (D) visa in the **issuing** state do not consume the allowance; days in other Schengen states do | `presence.ts` (`residence_permit_issuing_state`) | Same manual, §1: "Periods of stay authorised under a residence permit or a long-stay visa shall not be taken into account…" |
